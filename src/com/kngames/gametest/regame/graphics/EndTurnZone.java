...
public class EndTurnZone extends REGameZone {
	
	public static final String TAG = EndTurnZone.class.getSimpleName();
	public static final IDObject id = new IDObject(TAG);
	public String getName() { return id.getName(); }
	public int getId() { return id.getId(); }
	
	public EndTurnZone(Rect area) {
		super(area);
	}
	public EndTurnZone(int x, int y, int originCorner, float width, float height, int sizeMode) {
		super(x, y, originCorner, width, height, sizeMode);
	}
	public EndTurnZone(float x, float y, int originCorner, float width, float height, int sizeMode) {
		super(x, y, originCorner, width, height, sizeMode);
	}
	...
	public void draw(Canvas canvas) {
		...
		canvas.drawText("END TURN", area.left + 10, textLocation, paint);
	}
}
