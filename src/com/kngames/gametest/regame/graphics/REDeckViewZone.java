...
public class REDeckViewZone extends REGameZone {
	...
	private void init(Bitmap image, REViewZoneCallback callback) {
		...
		int cardHeight = this.height() - (2 * BORDER_WIDTH);
	...
	private void resetXPos() { currentXPos = 10; }
	...
	public void update() {
		...
		if (callback != null) {
			...
			for (int i = 0; i < cards.size(); i++) {
				cardPics.add(new TestRECard(getNextXPos() + cardBack.getWidth()/2, cardBack.getHeight()/2 + BORDER_WIDTH, 
	...
	public void handleDownTouch(float x, float y) {
		for (int i = cardPics.size() - 1; i >= 0; i--) {
			try {
				if (cardPics.get(i).isTouched(x, y, true)) {
